...
implements CallBack {
	...
	private int mTargetActivityThemeId;
...
//			mTargetContext.classLoaderReady(sLastClassLoader);
		...
		mTargetActivityThemeId = mActInfo.theme;
		...
		Log.d(TAG, "mThemeId             : " + mTargetActivityThemeId);
...
//		mTargetContext.packageNameReady(mActInfo.packageName);
		...
		Application app = InstalledAPks.getApplication(mApkPath);
		if (null == app) {
			if (!TextUtils.isEmpty(mApplicationClassName)) {
				try {
...
//do NOT edit this file, auto-generated by createTemplage.groovy from Target_Activity.java.template
					...
					appBaseContext.setTheme(appTheme);
...
//					ReflectUtil.ActivityReflectUtil.attachBaseContext(app, appBaseContext);
					
					((Host_Application)getApplication()).attachBundleAplication(app, appBaseContext);
					
					InstalledAPks.putApplication(mApkPath, (app));
				} catch (Exception e) {
					e.printStackTrace();
		...
		try {
			...
			if (mTargetActivityThemeId  > 0) {
			} else {
			}
			mTargetActivityThemeId = ReflectUtil.ResourceUtil.selectDefaultTheme(mResourceMerger, mTargetActivityThemeId, mActInfo.applicationInfo.targetSdkVersion);
			Log.d(TAG, "resolved activity theme: " + mTargetActivityThemeId);
			mTargetContext.setTheme(mTargetActivityThemeId);
			mTargetContext.themeReady(mTargetActivityThemeId);
